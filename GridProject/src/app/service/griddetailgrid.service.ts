...
//import { UpdateColumnDetailRequestModel } from '../interface/GridDetail';


@Injectable({
  providedIn: 'root',
})
export class GriddetailgridService {
  ...
  // employeeDetails api call
  GetEmployeeList(): Observable<any> {
    const httpOptionsToUse = this.getHttpOptions();

    // Assuming userId is part of the URL (adjust accordingly if it's a query parameter)
    const url = `${this.urlFor}api/Employee/GetEmployeeList`;
  ...
  GetUserProfile(userId: any): Observable<any> {
    ...
    return this.http
      .post(this.urlFor + `api/Employee/GetUserDetailById?userId=${userId}`, httpOptionsToUse)
      .pipe((response: any) => {
        return response;
      });
  }
